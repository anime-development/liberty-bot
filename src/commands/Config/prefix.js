...
module.exports = {
	...
	async command(client, message, args, extras) {
        ...
        if(newPrefix === extras.data.Prefix) return message.reply({ content: `The new prefix cannot be the same as the old one!`, allowedMentions: { repliedUser: false } });
        try {
            await extras.data.updateOne({ Prefix: String(newPrefix) });
